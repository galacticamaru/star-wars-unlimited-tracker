...
interface VariantWantSectionProps {
  printings: Printing[];
  onQuantityChange?: (cardPrintingId: number, quantity: number) => void;
}

export function VariantWantSection({ printings, onQuantityChange }: VariantWantSectionProps) {
  // Initialize counts map from RSC-fetched want quantity per printing (D-07 — no client fetch)
  const [counts, setCounts] = useState<Record<number, number>>(
    Object.fromEntries(printings.map(p => [p.id, p.quantity]))
  );
  ...
  const updateVariant = async (cardPrintingId: number, newCount: number) => {
    ...
    try {
      const res = await fetch('/api/binder/wants', {
        ...
        body: JSON.stringify({ cardPrintingId, quantity: val }),
      });
      if (!res.ok) {
  ...
  return (
    // Container — matches VariantCollectionSection/VariantTradeSection container exactly (UI-SPEC Surface 2)
    <div className="flex flex-col gap-2 p-4 bg-muted/50 rounded-lg border border-border">
      ...
      {/* Per-variant rows — always enabled, no ownership gating (D-06/D-07) */}
        ...
        return (
          <div key={printing.id} className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={count === 0}
              aria-label={`Decrease ${printing.variantType} want quantity`}
            >
            ...
            <Input
              ...
              aria-label={`${printing.variantType} want quantity`}
            />
            ...
            <Button
              ...
              size="icon"
              onClick={() => updateVariant(printing.id, count + 1)}
              aria-label={`Increase ${printing.variantType} want quantity`}
            >
              <Plus className="size-4" />
            </Button>

            {/* Status indicator — Wanted / Not wanted (UI-SPEC §Interaction Contract) */}
            {count > 0 ? (
              <span className="text-sm font-bold text-primary">Wanted</span>
            ) : (
              <span className="text-sm font-normal text-muted-foreground">Not wanted</span>
            )}
          </div>
        );
      })}
