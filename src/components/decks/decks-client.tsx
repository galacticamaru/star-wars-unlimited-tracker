...
interface DecksClientProps {
    initialDecks: Deck[];
    initialWantList: WantListEntry[];
}

const TYPE_ORDER = ['Leader', 'Base', 'Unit', 'Event', 'Upgrade'];

export function DecksClient({ initialDecks, initialWantList }: DecksClientProps) {
  const [decks, setDecks] = useState<Deck[]>(initialDecks);
  const [newDeckName, setNewDeckName] = useState('');
  const [wantList] = useState<WantListEntry[]>(initialWantList);
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      {decks.length > 0 && wantList.length > 0 && (
        <div className="mt-12">
